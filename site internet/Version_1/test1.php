...
// nom = "name"		mail = "email"		mdp = "MDP"		mdp2 = "MDP2"
$nameErr = $emailErr = $mdpErr = $mdp2Err = $telErr = $faxErr ="";
$name = $email = $mdp = $mdp2 = $tel = $fax ="";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") 
{
  ...
  if (empty($_POST["tel"])) 
  {
    $telErr = "Telephon is required"."<br>";
    echo $telErr;
  } else {
    $tel = $_POST["tel"];
    if (!preg_match("/^[0-9 ]{10}$/",$tel)) {
      $telErr = "Telephon number invalid"."<br>"; 
      echo $telErr;
    }
  }
  ...
  if (empty($_POST["fax"])) 
  {
    $faxErr = "Fax is required"."<br>";
    echo $faxErr;
  } else {
    $fax = $_POST["fax"];
    if (!preg_match("/^[0-9 ]{10}$/",$fax)) {
      $faxErr = "Fax number invalid"."<br>"; 
      echo $faxErr;
    }
  }
  ...
  if (empty($_POST["MDP"]))
  {
  	$mdpErr = "Mot de passe required"."<br>";
    echo $mdpErr;
  } else {
  	$mdp = $_POST["MDP"];
  	if(!preg_match('/^(?=.*\d)(?=.*[A-Z])(?=.*[a-z])[0-9A-Za-z]{8,12}$/',$mdp)){
  		$mdpErr = "Invalid password"."<br>";
      echo $mdpErr;
  ...
  if (empty($_POST["MDP2"]))
  {
  	$mdp2Err = "Confirm password"."<br>";
    echo $mdp2Err;
  } else {
  	$mdp2 = $_POST["MDP2"];
  	if ($mdp2!=$mdp){
  		$mdp2Err = "Password error"."<br>";
      echo $mdp2Err;
  	}
  }
}

	if ($mdpErr =='' && $mdp2Err =='' &&  $emailErr =='' &&  $nameErr =='' && $telErr =='' && $faxErr =='' ){
echo "Merci de votre inscription ".$name."<br><br>";

	   $reponse = $bdd->query("INSERT INTO client VALUES(null,'$name','$email','$mdp','$tel','$fax',null,null,null,null,null)");
     
	   $reponse->closeCursor();
   }else{
    echo "<br>"."Votre inscription a échouée"."<br><br>";
    }
...
<form class="">
            <div class="form-group"> <label>Email address</label>
              <input type="email" class="form-control" placeholder="Enter email"> <small class="form-text text-muted">We'll never share your email with anyone else.</small> </div>
            <div class="form-group"> <label>Password</label>
              <input type="password" class="form-control" placeholder="Password"> </div>
            <button type="submit" class="btn btn-primary" id="Connection">Connection</button>
          </form>
          <div class="blockquote">
            <div class="blockquote-footer">Pas de compte? Crée le votre ici:&nbsp;
              <br> </div>
          </div>
          <a class="btn btn-primary" href="CreateAccount.html">Création de compte
            <br> </a>
      </div>
    </div>
  </div>
